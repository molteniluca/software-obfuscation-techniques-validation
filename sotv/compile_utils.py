...
def compile_program(source_file, executable_elf, asm, destination_folder, O=0):
    ...
    try:
        compile_exec(source_file, os.path.join(destination_folder, executable_elf), O=O)
        compile_asm_nosymbols(source_file, os.path.join(destination_folder, asm))
...
def obfuscate_bench(input_path: str, entry_point: str, rep_scrambling: int, rep_obfuscate: int, rep_garbage: int, heat_value: int):
    ...
    obfuscate_bench_2(input_path, input_path+".s", entry_point, 0, rep_obfuscate, 1, rep_garbage, heat_value)
